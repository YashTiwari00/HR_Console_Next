...
function computeWeightedScore(goals: OrgGoalItem[]) {
...
export default function HrTeamAnalyticsPage() {
  ...
  const [orgCheckIns, setOrgCheckIns] = useState<CheckInItem[]>([]);
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      const [nextGoals, nextMembers, nextCheckIns] = await Promise.all([
        fetchGoals("all"),
      ...
      const employeeIds = Array.from(
        new Set(
          [
            ...nextMembers.map((member) => String(member.$id || "").trim()),
            ...nextGoals.map((goal) => String(goal.employeeId || "").trim()),
          ].filter(Boolean)
        )
      );
      ...
      setOrgCheckIns(nextCheckIns);
      setTrajectoryByEmployeeId(nextTrajectoryByEmployeeId);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to load HR analytics.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const cycleOptions = useMemo(() => {
    const values = Array.from(new Set(orgGoals.map((goal) => goal.cycleId).filter(Boolean)));
  ...
  const analyticsRows = useMemo(() => {
    const goalsInWindow = orgGoals.filter((goal) =>
    ...
    const filteredGoals = goalsInWindow.filter((goal) => {
      ...
      const member = memberInfoById.get(employeeId);
    ...
    let rank = 0;
    return sorted.map((row): OrgAnalyticsRow => {
  ...
  }, [analyticsDepartmentFilter, analyticsTimeFilter, memberInfoById, orgGoals, selectedCycleId, trajectoryByEmployeeId]);
  ...
  const graphMetrics = useMemo(() => {
    ...
    const graphWidth = Math.max(760, analyticsRatedRows.length * barWidth + Math.max(0, analyticsRatedRows.length - 1) * barGap);
  ...
  const kpiSummary = useMemo(() => {
    ...
    const departmentGoals = orgGoals.filter((goal) => {
      const employeeId = goal.employeeId;
      if (!employeeId) return false;
      if (analyticsDepartmentFilter === "all") return true;
      const member = memberInfoById.get(employeeId);
    ...
    const goalsByEmployeeCycle = new Map<string, Map<string, OrgGoalItem[]>>();
    departmentGoals.forEach((goal) => {
      ...
      const cycleMap = goalsByEmployeeCycle.get(employeeId) || new Map<string, OrgGoalItem[]>();
  ...
  }, [analyticsDepartmentFilter, analyticsRatedRows, memberInfoById, orgGoals, selectedCycleId]);
    ...
    return {
      ...
      decliningTrends:
        decliningRows.length > 0
          ...
          : "No declining trajectory detected across the current ranked organization view.",
  ...
  }, [analyticsDepartmentFilter, analyticsRows, analyticsTimeFilter, memberInfoById, orgCheckIns, orgGoals, selectedCycleId]);
  ...
  const explainabilityByEmployeeId = useMemo<Record<string, ExplainabilityPayload>>(() => {
    ...
    const goalsByEmployeeId = new Map<string, OrgGoalItem[]>();
    filteredGoals.forEach((goal) => {
      const employeeId = String(goal.employeeId || "").trim();
      if (!employeeId) return;
    ...
    return payloadMap;
  }, [analyticsDepartmentFilter, analyticsRows, analyticsTimeFilter, memberInfoById, orgGoals, selectedCycleId]);
  ...
  const analyticsColumns = useMemo<DataTableColumn<OrgAnalyticsRow>[]>(
    () => [
      {
        key: "rank",
        header: "Rank",
  ...
  return (
    <Stack gap="4">
      ...
      <Card title="Organization Performance Analytics" description="Ranking based on weighted manager ratings and progress levels.">
        <Stack gap="3">
          <div className="flex flex-wrap items-center gap-2">
            <label className="caption text-[var(--color-text-muted)]" htmlFor="hr-analytics-department-filter">
              Department
            </label>
            <select
              id="hr-analytics-department-filter"
            ...
            <label className="caption text-[var(--color-text-muted)]" htmlFor="hr-analytics-time-filter">
              Time
            </label>
            <select
              id="hr-analytics-time-filter"
              className="rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1 body-sm text-[var(--color-text)]"
            ...
            {analyticsTimeFilter === "cycle" && (
              <>
                <label className="caption text-[var(--color-text-muted)]" htmlFor="hr-analytics-cycle-filter">
                  Cycle
                </label>
                <select
                  id="hr-analytics-cycle-filter"
                  className="rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1 body-sm text-[var(--color-text)]"
          ...
          {analyticsRatedRows.length > 0 && (
            <div className="rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="body-sm font-medium text-[var(--color-text)]">Graph (Weighted Rating)</p>
                <Button
                  size="sm"
                  variant="secondary"
          ...
          {analyticsRows.length > 0 && (
            <DataTable
              ...
              emptyMessage="No ranked organization records found for the selected filters."
              maxHeight={480}
            />
          )}
            ...
            <Stack gap="2">
              {insightsLoading && (
                <Alert
                  ...
                  description="Analyzing decision insights across the current ranked organization."
                />
              )}
              {!insightsLoading && (
                <>
                ...
                return (
                  <div key={item.employeeId} className="flex flex-col gap-2">
                    <Alert
                      ...
                      title={`${item.employeeName} • ${item.role} • ${item.department}`}
